...
const clearUserByEmail = async (email) => {
  ...
  test('registerUser should register a user with teacher role', async () => {
    ...
    assert.strictEqual(userRoleData.role, role, 'User role should be teacher');
    await fetch(`http://localhost:5000/api/deleteUser/${registeredUser.uid}`, {
        method: 'DELETE',
      });
  });
  
test('register teacher should fail with an invalid email', async () => {
